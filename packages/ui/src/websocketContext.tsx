...
export const WebSocketProvider = ({
   ...
   const url = process.env.socket_dev ?? process.env.socket_prod;
   ...
   const connect = () => {
      ...
      const ws = new WebSocket(
         `${process.env.NODE_ENV === "development" ? `ws://localhost:8080/?token=${token}` : `https://chat-app-sparkling-pond-3900.fly.dev/?token=${token}`}`,
      );
